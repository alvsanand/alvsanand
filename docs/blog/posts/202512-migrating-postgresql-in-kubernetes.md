...
More info on that can be found in the [CNPG PostgreSQL Container Images](https://cloudnative-pg.io/docs/1.28/database_import#the-monolith-type).
...
Next, I spun up a completely new cluster running **Postgres 17 on Trixie**. I used the `bootstrap.initdb.import` feature to pull data directly from the old cluster. More info on that can be found in the [CloudNativePG docs](https://cloudnative-pg.io/docs/1.28/database_import#the-monolith-type).
